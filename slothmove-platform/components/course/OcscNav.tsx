...
export function OcscNav({ course }: { course: CourseConfig }) {
  ...
  const isValidSubject =
    ...
    course.subjects.some((subject) => subject.id === segments[2]);
  ...
  return (
    <nav className="course-nav is-ocsc is-v3-nav">
      <div className="container course-nav-inner">
        <Link href={`/courses/${course.id}`} className="course-nav-brand">
          <img src="/pic/slothmove_mascot.png" alt="Sloth × OCSC" className="course-nav-brand-mascot" />
          <img src={course.theme.logo} alt={course.title} className="course-nav-brand-logo" />
          <span className="course-nav-brand-copy">
            <strong>Sloth <span className="course-nav-brand-x">×</span> <span className="course-nav-brand-accent">OCSC</span></strong>
          </span>
        </Link>

        <div className="course-nav-actions">
          <Link href={backHref} className="course-nav-action course-nav-home">
      ...
      >
        <div className="course-mobile-menu-head">
          <div>
            ...
            <span>OCSC · {course.title}</span>
          </div>
          <span className="course-mobile-menu-badge">ภาค ก</span>
        </div>
        <div className="course-mobile-menu-list">
          ...
          <button type="button" onClick={openDonate}>
            <span className="course-mobile-menu-icon">☕</span>
            <span><strong>เลี้ยงกาแฟ</strong><small>สนับสนุนให้คอร์สเปิดฟรีต่อไป</small></span>
            <i>→</i>
          </button>
          <a
            href="https://www.facebook.com/profile.php?id=61589670089745"
          ...
          >
            ...
            <span><strong>Facebook</strong><small>ติดตามอัปเดตจากเพจ</small></span>
            <i>→</i>
          </a>
          <Link href={examHref} className="is-primary" onClick={() => setMobileOpen(false)}>
            ...
            <span>
              ...
              <small>{isValidSubject ? 'เข้าสู่ลานฝึกวิชานี้' : 'เริ่มทำข้อสอบจำลองภาค ก.'}</small>
            </span>
            <i>→</i>
          </Link>
        </div>
