...
export function ComputerTrueFalseGame({
  items,
  ...
  function startGame() {
    setQuestionPool(
      buildDistinctRandomSession(
        distinctScope('computer-truefalse', courseId, subjectId, items.length),
  ...
  const isCorrect = selectedAnswer !== null && currentItem && selectedAnswer === currentItem.answer;

  function handlePick(answer: boolean) {
    if (hasAnswered || !currentItem) return;
    setSelectedAnswer(answer);
    setHasAnswered(true);
    if (currentItem.answer === answer) setScore((current) => current + 1);
  }
        ...
        <div className="max-w-3xl mx-auto px-4 py-10">
          ...
          <div className="tf-card bg-white dark:bg-zinc-900 rounded-3xl p-8 text-center relative overflow-hidden">
            ...
            <p className="text-sm md:text-base text-slate-600 dark:text-zinc-400 max-w-xl mx-auto leading-relaxed mb-6">
              ระบบจะโชว์ข้อความอธิบายของคำศัพท์หรือคำสั่ง 1 ประโยค แล้วให้ตัดสินว่าข้อความนั้นถูกหรือผิด
              โดยตัวลวงจะเลือกจากความหมายใกล้เคียงกันในหมวดเดียวกันเพื่อให้โจทย์แน่นขึ้น
            </p>
        ...
        <div className="max-w-3xl mx-auto px-4 py-10">
          <div className="tf-card bg-white dark:bg-zinc-900 rounded-3xl p-8 text-center relative overflow-hidden">
            ...
            <div className="flex gap-4 justify-center items-center">
              ...
              <Link href={`/courses/${courseId}/${subjectId}/practices`} className="tf-btn-start px-8 py-3.5 rounded-2xl text-base font-bold cursor-pointer text-white no-underline inline-block">← กลับไปลานฝึก</Link>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="tf-card bg-white dark:bg-zinc-900 rounded-3xl p-8 text-center relative overflow-hidden">
          ...
          <div style={{ height: 6, background: 'var(--color-border)', borderRadius: 3, overflow: 'hidden', marginBottom: 24 }}>
            <div style={{ height: '100%', width: `${((idx + (hasAnswered ? 1 : 0)) / questionPool.length) * 100}%`, background: 'linear-gradient(90deg, var(--color-primary) 0%, var(--color-accent) 100%)', transition: 'width 0.3s ease' }} />
          </div>
          <div className="mb-6">
            <span className="text-xs font-bold text-amber-600 dark:text-amber-400 tracking-wider uppercase mb-1 block">คำถามข้อที่ {idx + 1} จาก {questionPool.length}</span>
            <h2 className="font-display font-black text-3xl md:text-4xl my-4 text-slate-900 dark:text-zinc-50">{currentItem.word}</h2>
