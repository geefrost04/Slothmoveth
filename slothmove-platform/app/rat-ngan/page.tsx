...
function JobBoardContent() {
  ...
  const baseJobs = getActiveOcscJobs(JOBS_DATA, now);
  const educationOptions = EDUCATION_FILTERS.filter((item) =>
    baseJobs.some((job: any) => getEducationLevels(job['ระดับการศึกษา']).includes(item.id))
  );

  const filterCounts = {
    all: baseJobs.length,
    'ข้าราชการพลเรือน': baseJobs.filter((job: any) => (job['ประเภทงาน'] || '').includes('ข้าราชการ')).length,
    'พนักงานราชการ': baseJobs.filter((job: any) => (job['ประเภทงาน'] || '').includes('พนักงานราชการ')).length,
    'อื่นๆ': baseJobs.filter((job: any) => {
      const type = (job['ประเภทงาน'] || '').trim();
      return !type.includes('ข้าราชการ') && !type.includes('พนักงานราชการ');
    }).length,
  };
  ...
  const filteredJobs = baseJobs.filter((job: any) => {
    if (filter !== 'all') {
      const type = (job['ประเภทงาน'] || '').trim();
      if (filter === 'ข้าราชการพลเรือน' && !type.includes('ข้าราชการ')) return false;
      if (filter === 'พนักงานราชการ' && !type.includes('พนักงานราชการ')) return false;
      if (filter === 'อื่นๆ' && (type.includes('ข้าราชการ') || type.includes('พนักงานราชการ'))) return false;
    }

    if (educationFilter !== 'all') {
      const jobEducation = getEducationLevels(job['ระดับการศึกษา']);
    ...
    if (deferredQuery.trim()) {
      ...
      if (q === 'ไม่ต้องมีภาค ก.' || q === 'ไม่ต้องมีภาคก') {
        return !(job['วิธีการสรรหา'] || '').includes('สอบแข่งขัน');
      }
      if (q === 'ต้องมีภาค ก.' || q === 'ต้องมีภาคก' || q === 'ภาค ก' || q === 'ภาคก') {
        return (job['วิธีการสรรหา'] || '').includes('สอบแข่งขัน');
      }

      const position = (job['ตำแหน่ง'] || '').toLowerCase();
      const agency = (job['หน่วยงาน'] || '').toLowerCase();
      const ministry = (job['กระทรวง'] || '').toLowerCase();
      const province = (job['จังหวัด'] || '').toLowerCase();
      const type = (job['ประเภทงาน'] || '').toLowerCase();
      const method = (job['วิธีการสรรหา'] || '').toLowerCase();
  ...
  const sortedJobs = [...filteredJobs].sort((left: any, right: any) => {
    if (sort === 'date') {
      return (right['วันเริ่มสมัคร'] || '').localeCompare(left['วันเริ่มสมัคร'] || '');
    }

    return getCleanTitle(left['ตำแหน่ง'] || '').localeCompare(getCleanTitle(right['ตำแหน่ง'] || ''), 'th');
  });

  const totalPositions = filteredJobs.reduce((total, job: any) => {
    const value = parseInt(String(job['จำนวนรับ'] || '0').replace(/,/g, ''), 10);
    return total + (Number.isNaN(value) ? 0 : value);
  }, 0);
  const totalAgencies = new Set(filteredJobs.map((job: any) => job['หน่วยงาน']).filter(Boolean)).size;
  const totalMinistries = new Set(filteredJobs.map((job: any) => job['กระทรวง']).filter(Boolean)).size;
  const urgentCount = filteredJobs.filter(
    (job: any) => getStatus(job['วันเริ่มสมัคร'], job['วันหมดสมัคร'], now).id === 'urgent'
  ).length;
  const upcomingCount = filteredJobs.filter(
    (job: any) => getStatus(job['วันเริ่มสมัคร'], job['วันหมดสมัคร'], now).id === 'before'
  ).length;

  const topMinistries = Object.entries(
    filteredJobs.reduce<Record<string, number>>((accumulator, job: any) => {
      const ministry = (job['กระทรวง'] || '').trim();
  ...
  const latestStart = sortedJobs[0]?.['วันเริ่มสมัคร'];
          ...
          <div>
            ...
            <div className="mt-4 grid grid-cols-2 gap-2 lg:grid-cols-4">
              {[
                ...
                { label: 'ประกาศที่แสดงผล', value: sortedJobs.length.toLocaleString(), accent: '#17241d' },
                { label: 'หน่วยงานที่พบ', value: totalAgencies.toLocaleString(), accent: '#17241d' },
                { label: 'ใกล้ปิดรับ', value: urgentCount.toLocaleString(), accent: '#b77933' },
              ].map((metric) => (
                <div
                  key={metric.label}
                  className="rounded-[14px] border border-[#ebe4da] bg-[#faf8f4] px-3 py-2.5 dark:border-[#37374e] dark:bg-[#252540]"
                >
                  <div className="text-[9px] font-bold uppercase tracking-[0.1em] text-slate-400 font-display">
                    {metric.label}
                  </div>
                  <div className="mt-1 font-display text-[1.35rem] font-extrabold leading-none" style={{ color: metric.accent }}>
                    {metric.value}
                  </div>
                </div>
              ))}
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              ...
              {activeRefinements.length > 0 || query || filter !== 'all' || educationFilter !== 'all' || sort !== 'date' ? (
                <button
                  type="button"
      ...
      {sortedJobs.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-4 pb-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          {sortedJobs.map((job: any) => {
            const jobType = job['%E0%B8%9B%E0%B8%A3%E0%B8%B5%E0%B8%B0%E0%B9%80%E0%B8%A0%E0%B8%97%E0%B8%87%E0%B8%B2%E0%B8%99'] || job['ประเภทงาน'] || '';
            ...
            const typeLightBg = isCivil ? '#e8f5ef' : isGovEmployee ? '#fdf3e8' : '#f0ebe4';

            const status = getStatus(job['%E0%B8%A7%E0%B8%B1%E0%B8%99%E0%B9%80%E0%B8%A3%E0%B8%B4%E0%B9%88%E0%B8%A1%E0%B8%AA%E0%B8%A1%E0%B8%B1%E0%B8%84%E0%B8%A3'] || job['วันเริ่มสมัคร'], job['%E0%B8%A7%E0%B8%B1%E0%B8%99%E0%B8%AB%E0%B8%A1%E0%B8%94%E0%B8%AA%E0%B8%A1%E0%B8%B1%E0%B8%84%E0%B8%A3'] || job['วันหมดสมัคร'], now);
            let statusClasses = 'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300';
            if (status.id === 'open') {
              statusClasses = 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-400';
            } else if (status.id === 'urgent' || status.id === 'before') {
              statusClasses = 'bg-amber-50 text-amber-700 dark:bg-amber-950/30 dark:text-amber-400';
            } else if (status.id === 'closed') {
              statusClasses = 'bg-rose-50 text-rose-700 dark:bg-rose-950/30 dark:text-rose-400';
            }

            const ministryColor = getMinistryColor(job['%E0%B8%81%E0%B8%A3%E0%B8%B0%E0%B8%17%E0%B8%A3%E0%B8%27%E0%B8%87'] || job['กระทรวง'] || '');
            const cleanTitle = (job['%E0%B8%95%E0%B8%B3%E0%B9%81%E0%B8%20%E0%B9%88%E0%B8%87'] || job['ตำแหน่ง'] || '')
              .replace(/^รับสมัครบุคคลเพื่อเลือกสรรและจัดจ้างเป็นพนักงานราชการ ตำแหน่ง /, '')
              .replace(/^ตำแหน่ง/, '')
              .trim();
            const requiresOcsc = (job['%E0%B8%A7%E0%B8%B4%E0%B8%98%E0%B8%B5%E0%B8%81%E0%B8%B2%E0%B8%A3%E0%B8%AA%E0%B8%A3%E0%B8%A3%E0%B8%AB%E0%B8%B2'] || job['วิธีการสรรหา'] || '').includes('สอบแข่งขัน');
            const province = job['%E0%B8%88%E0%B8%B1%E0%B8%87%E0%B8%AB%E0%B8%A7%E0%B8%B1%E0%B8%94'] || job['จังหวัด'] || '';
            const showProvince = province && province !== 'กรุงเทพมหานคร' && province !== 'ไม่มีข้อมูล' && province !== 'หลายจังหวัด';

            let salaryText = '';
            const minSalary = job['%E0%B9%80%E0%B8%87%E0%B8%B4%E0%B8%99%E0%B9%80%E0%B8%94%E0%B8%B7%E0%B8%AD%E0%B8%99%E0%B8%95%E0%B8%B2%E0%B9%88%E0%B8%AA%E0%B8%B8%E0%B8%95'] || job['เงินเดือนต่ำสุด'];
            const maxSalary = job['%E0%B9%80%E0%B8%87%E0%B8%B4%E0%B8%99%E0%B9%80%E0%B8%94%E0%B8%B7%E0%B8%AD%E0%B8%99%E0%B8%AA%E0%B8%B9%E0%B8%87%E0%B8%AA%E0%B8%B8%E0%B8%95'] || job['เงินเดือนสูงสุด'];
            if (minSalary && maxSalary) {
              salaryText = `${parseInt(minSalary, 10).toLocaleString()} - ${parseInt(maxSalary, 10).toLocaleString()} บาท`;
            } else if (minSalary) {
              salaryText = `${parseInt(minSalary, 10).toLocaleString()} บาท`;
            }

            let accentBorder = 'border-t-[4px] border-t-[#67747c]';
            if (isCivil) {
              accentBorder = 'border-t-[4px] border-t-[#3d8c6c]';
            } else if (isGovEmployee) {
              accentBorder = 'border-t-[4px] border-t-[#c97d3a]';
            }

            const logoUrl = job['%E0%B9%82%E0%B8%A5%E0%B9%82%E0%B8%82%E0%B9%89%E0%B8%AB%E0%B8%99%E0%B9%88%E0%B8%A7%E0%B8%A2%E0%B8%87%E0%B8%B2%E0%B8%99'] || job['โลโก้หน่วยงาน'] || '';
            ...
            // Urgency badge — gradient by days remaining.
            // Closed = grey strike-through, >10 days = green/grey, ≤10 = orange, ≤3 = red.
            ...
            return (
              ...
              >
                {/* Type label — top-left corner (over the border-left strip) */}
                <span
                ...
                <div className="flex flex-1 flex-col p-5">
                    ...
                    <div className="mt-2.5 text-center">
                      ...
                      {job['กระทรวง'] ? (
                        <div
                          ...
                          title={job['กระทรวง']}
                        >
                          {job['กระทรวง'].replace('กระทรวง', '').trim()}
                        </div>
                      ) : null}
                    </div>
                  </div>

                  {/* Job Title: 16px / 500, the focal point */}
                  ...
                  {/* Sub-info: 2 rows for visual balance */}
                  <div className="mb-1.5 flex flex-wrap items-center justify-center gap-x-3 gap-y-1.5 text-[11px] text-slate-600 dark:text-slate-400">
                    ...
                    <span className="inline-flex items-center gap-1 font-semibold text-slate-700 dark:text-slate-300">
                      ...
                      {job['จำนวนรับ'] ? parseInt(job['จำนวนรับ'], 10).toLocaleString() : '1'} อัตรา
                    </span>
                  </div>
                  ...
                  {/* Salary — calmer, calm-editorial style */}
                  ...
                  {/* Days-remaining progress bar — SlothMove themed */}
